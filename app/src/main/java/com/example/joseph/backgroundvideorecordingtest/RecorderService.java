...
import static com.example.joseph.backgroundvideorecordingtest.MainActivity.VID_DIR_PATH;
...
public class RecorderService extends Service implements SurfaceHolder.Callback {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        Log.i("TESTING", "onStartCmd called");
        if (intent != null) {
            videoPath = intent.getStringExtra(INTENT_VIDEO_PATH);
        }
        else {
            videoPath = VID_DIR_PATH;
        }
    ...
    public void surfaceCreated(final SurfaceHolder surfaceHolder) {
        Log.i("TESTING", "surfaceCreated called");
    ...
    private void setupSurface(SurfaceHolder surfaceHolder) {
        Log.i("TESTING", "setupSurface called");
        try {
            ...
            if (mServiceCamera != null) {
                ...
                File directory = new File(Environment.getExternalStorageDirectory().getPath() + videoPath);
                if (!directory.exists())
                    directory.mkdirs();
                long currentTime = System.currentTimeMillis();
                String fileNameString = "videooutput" + currentTime + ".mp4";
                String uniqueOutFile = Environment.getExternalStorageDirectory().getPath() + videoPath
                        + fileNameString;
